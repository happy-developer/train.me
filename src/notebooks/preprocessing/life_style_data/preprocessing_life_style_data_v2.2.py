...
selected_features = [
    ...
    # Core
    "Age", "Weight (kg)", "Gender", "Experience_Level"
]
...
assert {"Age", "Weight (kg)", "Gender_1.0", "Experience_Level"} <= set(df_feat_raw.columns)

# X/y bruts (unités réelles pour Age / Weight / Calories_Burned, binaire pour Gender_1.0)
X_raw = df_feat_raw[["Age", "Weight (kg)", "Gender_1.0", "Experience_Level"]].copy()
...
X_val_scaled   = X_val[feature_names].values
X_test_scaled  = X_test[feature_names].values
...
feature_schema = {
    ...
    "model_version": "v2.2-minimal",  # ajuste si tu es sur un set élargi
    "created_at": datetime.utcnow().isoformat() + "Z",
