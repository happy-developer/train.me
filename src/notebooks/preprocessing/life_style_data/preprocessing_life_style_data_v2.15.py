...
selected_features = [
    ...
    # "Age", "Weight (kg)", "Gender", "Experience_Level", "Workout_Frequency (days/week)", "Session_Duration (hours)", "Height (m)", "Max_BPM", "Avg_BPM", "Resting_BPM", "Difficulty Level", "Body Part", "Equipment Needed", "Water_Intake (liters)", "Fat_Percentage", "diet_type"
]
...
plt.title("Corrélation des 20 variables les plus liées à Calories_Burned")
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"Avg_BPM", "Max_BPM"}.issubset(df.columns):
        ...
        # feature_cols_raw.append("BPM_effort_ratio")
...
if search_best_params:
    ...
    print("\nComparatif rapide :")
    print(f"Baseline RF → MAE: {mae_base:.4f}, RMSE: {rmse_base:.4f}, R²: {r2_base:.4f}")
    print(f"Best RF     → MAE: {mae_best:.4f}, RMSE: {rmse_best:.4f}, R²: {r2_best:.4f}")
...
if search_best_params:
    ...
    search_bag = RandomizedSearchCV(
        ...
        n_iter=40,              # augmente si tu veux explorer plus
        scoring="r2",
        cv=3,
        n_jobs=-1,
        verbose=1,
    ...
    print("\nComparatif rapide :")
    print(f"Bagging baseline → MAE: {mae_base:.4f}, RMSE: {rmse_base:.4f}, R²: {r2_base:.4f}")
    print(f"Bagging best     → MAE: {mae_best:.4f}, RMSE: {rmse_best:.4f}, R²: {r2_best:.4f}")
...
if search_best_params:
    ...
    print(f"GB baseline → MAE: {mae_base:.4f}, RMSE: {rmse_base:.4f}, R²: {r2_base:.4f}")
    ...
    print("\nComparatif rapide :")
    print(f"GB baseline  → MAE: {mae_base:.4f}, RMSE: {rmse_base:.4f}, R²: {r2_base:.4f}")
    print(f"GB best      → MAE: {mae_best:.4f}, RMSE: {rmse_best:.4f}, R²: {r2_best:.4f}")
...
manual_specs = {
    ...
    "Height (m)": {"type": "number", "min": 1.50, "max": 2.30},
    "Fat_Percentage": {"type": "number", "min": 11.0, "max": 35.0},
    "Max_BPM": {"type": "number", "min": 159, "max": 200},
    "Avg_BPM": {"type": "number", "min": 119, "max": 170},
    "Resting_BPM": {"type": "number", "min": 49.0, "max": 75.0},
    ...
    "Body Part": {                    
        "type": "string",
        "enum": ["Abs", "Arms", "Back", "Chest", "Forearms", "Legs", "Shoulders"],
    },
     "Equipment Needed": {                  
        "type": "string",
        "enum": [
     ...
     # ✅ Nouvelle spec pour diet_type
    "diet_type": {
        "type": "string",
        "enum": ["Balanced", "Keto", "Low-Carb", "Paleo", "Vegan", "Vegetarian"],
    },
}
...
feature_schema = {
    ...
    "model_version": "v2.15-minimal",  # ajuste si tu es sur un set élargi
    "created_at": datetime.utcnow().isoformat() + "Z",
