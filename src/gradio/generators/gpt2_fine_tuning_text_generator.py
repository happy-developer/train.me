...
class GPT2_FineTuningTextGenerator:
    ...
    ) -> "GPT2_FineTuningTextGenerator":
        """
        Charge (ou récupère en cache) un générateur GPT-2 *fine-tuné*
        depuis un dossier `model_path` (local ou NAS).
        """
        model_path = Path(model_path)

        print(f"[GPT2_FT] Loading from: {model_path}")
    ...
    # ------------------------------------------------------------------
    # Fonctions “bonus” si tu veux les réutiliser
    # (copiées/collées de la version distillée)
    # ------------------------------------------------------------------
